object Versions {
  val http4s = "0.23.33"
  val http4sBlazeServer = "0.23.17"
  val http4sBlazeClient = "0.23.17"
  val catsCore = "2.13.0"
  val catsEffect = "3.6.3"
  val circe = "0.14.15-17-64ed324-20260319T214619Z-SNAPSHOT"
  val circeRefined = "0.15.1"
  val circeGenericExtras = "0.14.3"
  val circeYaml = "0.15.2"
  val helidon = "4.0.10"
  val sttp = "3.11.0"
  val sttp4 = "4.0.14"
  val sttpModel = "1.7.17+11-535e7c5b+20260212-1606-SNAPSHOT"
  val sttpShared = "1.5.0+70-e1c9b0e0+20260212-1925-SNAPSHOT"
  val sttpApispec = "0.11.10+4-29ef900a+20260319-2248-SNAPSHOT"
  val akkaHttp = "10.2.10"
  val akkaStreams = "2.6.20"
  val pekkoHttp = "1.2.0"
  val pekkoStreams = "1.2.0"
  val swaggerUi = "5.28.0"
  val upickle = "4.4.2"
  val upickle3 = "3.3.1"
  val playJson = "3.0.1"
  val play29Json = "3.0.6"
  val finatra = "24.2.0"
  val catbird = "21.12.0"
  val json4s = "4.0.7"
  val metrics4Scala = "4.3.7"
  val nettyReactiveStreams = "3.0.6"
  val ox = "1.0.2"
  val reactiveStreams = "1.0.4"
  val sprayJson = "1.3.6"
  val scalaCheck = "1.18.1"
  val scalaTest = "3.2.18"
  val scalaTags = "0.13.1"
  val scalaTestPlusScalaCheck = "3.2.19.0"
  val refined = "0.11.3"
  val iron = "3.0.3"
  val enumeratum = "1.9.0"
  val zio = "2.1.23"
  val zioHttp = "3.7.4"
  val zioInteropCats = "23.1.0.12"
  val zioInteropReactiveStreams = "2.0.2"
  val zioJson = "0.7.44"
  val playClient = "3.0.10"
  val playServer = "3.0.10"
  val play29Client = "2.2.14"
  val play29Server = "2.9.9"
  val tethys = "0.29.5"
  val vertx = "4.5.24"
  val jsScalaJavaTime = "2.6.0"
  val nativeScalaJavaTime = "2.6.0"
  val jwtScala = "10.0.4"
  val derevo = "0.14.0"
  val newtype = "0.4.4"
  val monixNewtype = "0.3.0"
  val zioPrelude = "1.0.0-RC45"
  val awsLambdaInterface = "2.6.0"
  val armeria = "1.34.2"
  val scalaJava8Compat = "1.0.2"
  val scalaCollectionCompat = "2.13.0"
  val fs2 = "3.12.2"
  val decline = "2.5.0"
  val quicklens = "1.9.12"
  val openTelemetry = "1.58.0"
  val openTelemetrySemconvVersion = "1.37.0"
  val mockServer = "5.15.0"
  val dogstatsdClient = "4.4.5"
  val nettyAll = "4.2.9.Final"
  val logback = "1.5.25"
  val slf4j = "2.0.17"
  val jsoniter = "2.35.3"
  val otel4s = "0.14.0"
}
